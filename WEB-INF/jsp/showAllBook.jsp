					...
					<tbody>
						<c:forEach items="${requestScope.pageMsg.lists}" var="book">
							<tr>
								...
								<td>${book.title }</td>
								<td>${book.author }</td>
								<td><c:if test="${book.categoryId==1 }">Novel</c:if> <c:if
										test="${book.categoryId==2 }">Literature</c:if> <c:if
										test="${book.categoryId==3 }">Animation</c:if> <c:if
										test="${book.categoryId==4 }">Culture</c:if> <c:if
										test="${book.categoryId==5 }">Biography</c:if> <c:if
										test="${book.categoryId==6 }">Art</c:if> <c:if
										test="${book.categoryId==7 }">Puffin Literature</c:if> <c:if
										test="${book.categoryId==8 }">Ancient Book</c:if> <c:if
										test="${book.categoryId==9 }">History</c:if></td>
								<td>${book.price }</td>
								...
								<td><a class="layui-btn"
									...
									href="bookDetail?id=${book.id }">Query</a></td>
								<td><a class="layui-btn"
								...
								<td><a class="layui-btn" id="soldBook"
									...
									href="deleteBook?id=${book.id }">Out of shelves</a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
